...
export default async function CvesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  for (const key of ["search", "severity", "exploited", "publicly_disclosed", "impact", "product_family", "product_category"]) if (params[key]) qs.set(key, params[key]!);
  ...
  qs.set("limit", "50");
  const apiCves = await getJson<Cve[]>(`/cves?${qs}`, []);
  ...
  const activeFilters = [["Search", params.search], ["Severity", severity], ["Release", release], ["Exploited", params.exploited], ["Public", params.publicly_disclosed], ["KEV", kev === "true" ? "CISA KEV only" : undefined], ["Min EPSS", minEpss], ["Min CVSS", minCvss], ["Impact", params.impact], ["Product family", params.product_family], ["Product category", params.product_category], ["CVSS bucket", params.cvss_bucket], ["Priority", params.priority], ["NVD", params.nvd], ["Sort", params.sort]].filter(([, value]) => Boolean(value));
