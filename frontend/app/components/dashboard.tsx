...
export type ProductSummary = { product_family: string; product_category: string; cve_count: number; critical_count: number; kev_count: number; high_epss_count: number; average_cvss_score: number | null };
export type ProductCategory = { product_category: string; cve_count: number; critical_count: number; kev_count: number; high_epss_count: number; average_cvss_score: number | null };
export type ProductRiskRanking = ProductSummary & { risk_score: number; risk_level: string };
export type ProductMonthlyDelta = { product_family: string; product_category: string; current_release: string; previous_release: string; current_cve_count: number; previous_cve_count: number; delta_cve_count: number; current_critical_count: number; previous_critical_count: number; delta_critical_count: number; current_kev_count: number; previous_kev_count: number; delta_kev_count: number; current_high_epss_count: number; previous_high_epss_count: number; delta_high_epss_count: number; delta_priority_score: number };
...
export type Stats = {
  ...
  top_epss_cves?: TopEpssCve[] | null; kev_cves?: KevCve[] | null; top_product_families?: ProductSummary[] | null; top_product_categories?: ProductCategory[] | null;
};
...
export function ProductFamiliesPanel({ products }: { products: ProductSummary[] | null | undefined }) { const rows = products ?? []; const max = Math.max(...rows.map((row) => safeNumber(row.cve_count)), 0); return <section className="soc-card"><h2 className="panel-title">Most Affected Products</h2>{rows.length === 0 ? <p className="empty-state mt-4">No product intelligence available yet.</p> : <div className="mt-4 space-y-3">{rows.slice(0, 10).map((row) => { const width = max > 0 ? Math.max((safeNumber(row.cve_count) / max) * 100, 4) : 0; return <Link key={`${row.product_family}-${row.product_category}`} href={`/cves?product_family=${encodeURIComponent(row.product_family)}`} className="block rounded-xl border border-slate-800/80 bg-slate-950/60 p-3 hover:border-cyan-400/30 hover:bg-cyan-400/10"><div className="flex items-center justify-between gap-3"><div><p className="font-bold text-slate-100">{row.product_family}</p><p className="text-xs text-slate-500">{row.product_category}</p></div><p className="text-lg font-black tabular-nums text-cyan-200">{formatCount(row.cve_count)}</p></div><div className="mt-2 h-2 rounded-full bg-slate-800"><div className="h-full rounded-full bg-cyan-400 shadow-[0_0_12px_rgba(34,211,238,0.55)]" style={{ width: `${width}%` }} /></div><div className="mt-2 grid grid-cols-3 gap-2 text-xs text-slate-400"><span>Critical <b className="text-rose-200">{formatCount(row.critical_count)}</b></span><span>KEV <b className="text-violet-200">{formatCount(row.kev_count)}</b></span><span>High EPSS <b className="text-orange-200">{formatCount(row.high_epss_count)}</b></span></div></Link>; })}</div>}</section>; }


export function ProductRiskRankingPanel({ products }: { products: ProductRiskRanking[] | null | undefined }) { const rows = products ?? []; const max = Math.max(...rows.map((row) => safeNumber(row.risk_score)), 0); return <section className="soc-card"><h2 className="panel-title">Top Products To Patch</h2>{rows.length === 0 ? <p className="empty-state mt-4">No product risk ranking available.</p> : <div className="mt-4 space-y-3">{rows.slice(0, 10).map((row) => { const width = max > 0 ? Math.max((safeNumber(row.risk_score) / max) * 100, 4) : 0; return <Link key={`${row.product_family}-${row.product_category}`} href={`/cves?product_family=${encodeURIComponent(row.product_family)}`} className="block rounded-xl border border-slate-800/80 bg-slate-950/60 p-3 hover:border-cyan-400/30 hover:bg-cyan-400/10"><div className="flex items-center justify-between gap-3"><div><p className="font-bold text-slate-100">{row.product_family}</p><p className="text-xs text-slate-500">{row.product_category}</p></div><div className="text-right"><p className="text-lg font-black tabular-nums text-cyan-200">{safeNumber(row.risk_score).toFixed(1)}</p><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">{row.risk_level}</p></div></div><div className="mt-2 h-2 rounded-full bg-slate-800"><div className="h-full rounded-full bg-cyan-400 shadow-[0_0_12px_rgba(34,211,238,0.55)]" style={{ width: `${width}%` }} /></div><div className="mt-2 grid grid-cols-3 gap-2 text-xs text-slate-400"><span>Critical <b className="text-rose-200">{formatCount(row.critical_count)}</b></span><span>KEV <b className="text-violet-200">{formatCount(row.kev_count)}</b></span><span>High EPSS <b className="text-orange-200">{formatCount(row.high_epss_count)}</b></span></div></Link>; })}</div>}</section>; }


export function ProductMonthlyDeltaPanel({ products }: { products: ProductMonthlyDelta[] | null | undefined }) { const rows = products ?? []; const comparison = rows[0] ? `${rows[0].current_release} vs ${rows[0].previous_release}` : "No release comparison available"; const max = Math.max(...rows.map((row) => Math.abs(safeNumber(row.delta_priority_score))), 0); return <section className="soc-card"><div className="flex items-start justify-between gap-3"><div><h2 className="panel-title">Biggest Product Changes</h2><p className="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{comparison}</p></div></div>{rows.length === 0 ? <p className="empty-state mt-4">No product monthly delta data available.</p> : <div className="mt-4 space-y-3">{rows.slice(0, 10).map((row) => { const width = max > 0 ? Math.max((Math.abs(safeNumber(row.delta_priority_score)) / max) * 100, 4) : 0; return <Link key={`${row.product_family}-${row.product_category}`} href={`/cves?product_family=${encodeURIComponent(row.product_family)}`} className="block rounded-xl border border-slate-800/80 bg-slate-950/60 p-3 hover:border-cyan-400/30 hover:bg-cyan-400/10"><div className="flex items-center justify-between gap-3"><div><p className="font-bold text-slate-100">{row.product_family}</p><p className="text-xs text-slate-500">{row.product_category}</p></div><div className="text-right"><p className="text-lg font-black tabular-nums text-cyan-200">{formatCount(row.delta_priority_score)}</p><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Priority Δ</p></div></div><div className="mt-2 h-2 rounded-full bg-slate-800"><div className="h-full rounded-full bg-cyan-400 shadow-[0_0_12px_rgba(34,211,238,0.55)]" style={{ width: `${width}%` }} /></div><div className="mt-2 grid grid-cols-4 gap-2 text-xs text-slate-400"><span>CVEs <b className="text-cyan-200">{formatCount(row.delta_cve_count)}</b></span><span>Critical <b className="text-rose-200">{formatCount(row.delta_critical_count)}</b></span><span>KEV <b className="text-violet-200">{formatCount(row.delta_kev_count)}</b></span><span>High EPSS <b className="text-orange-200">{formatCount(row.delta_high_epss_count)}</b></span></div></Link>; })}</div>}</section>; }

export function ProductCategoriesPanel({ categories }: { categories: ProductCategory[] | null | undefined }) { const buckets = (categories ?? []).map((row) => ({ label: row.product_category, count: row.cve_count })); return <DonutChart title="Product Categories" buckets={buckets} param="product_category" />; }
