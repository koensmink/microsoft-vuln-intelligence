...
const severityColors: Record<string, string> = { Critical: "#fb7185", Important: "#fb923c", High: "#fb923c", Moderate: "#facc15", Low: "#22c55e", Unknown: "#64748b", None: "#64748b" };
...
export function DonutChart({ title, buckets, param }: { title: string; buckets: CountBucket[] | null | undefined; param: string }) {
  ...
  let cursor = 0;
  const gradient = total > 0 ? rows.map((row, index) => { const start = cursor; const size = (row.count / total) * 100; cursor += size; return `${colorFor(row.label, index)} ${start}% ${cursor}%`; }).join(", ") : "#334155 0% 100%";

  return <section className="soc-card"><h2 className="panel-title">{title}</h2><div className="mt-5 grid gap-5 sm:grid-cols-[10rem_1fr] sm:items-center"><Link aria-label={`${title} details`} href="/cves" className="relative grid h-40 w-40 place-items-center rounded-full shadow-2xl shadow-cyan-950/30" style={{ background: `conic-gradient(${gradient})` }}><div className="grid place-items-center rounded-full border border-slate-700/80 bg-[#06101f] text-center" style={{ width: "5.5rem", height: "5.5rem" }}><span className="text-2xl font-black tabular-nums">{formatCount(total)}</span><span className="-mt-6 text-[0.62rem] uppercase tracking-[0.2em] text-slate-500">CVEs</span></div></Link><div className="space-y-2">{rows.length === 0 || total === 0 ? <p className="empty-state">No chart data available.</p> : rows.map((row, index) => <Link href={`/cves?${param}=${encodeURIComponent(row.label)}`} key={row.label} className="grid grid-cols-[0.75rem_1fr_auto] items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-cyan-400/10"><span className="block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: colorFor(row.label, index) }} /><span className="truncate text-slate-300">{row.label}</span><span className="font-bold tabular-nums text-slate-100">{formatCount(row.count)}</span></Link>)}</div></div></section>;
}
...
export function BarChart({ title, buckets, param = "", tone = "blue" }: { title: string; buckets: CountBucket[] | null | undefined; param?: string; tone?: Tone }) {
  ...
  const fill = toneStyles[tone].fill;

  return <section className="soc-card"><h2 className="panel-title">{title}</h2><div className="mt-5 flex items-end gap-3 border-b border-l border-slate-700/70 px-3 pb-2" style={{ height: "15rem" }}>{rows.length === 0 || max === 0 ? <p className="empty-state w-full self-center">No chart data available.</p> : rows.map((row) => { const heightPct = Math.max((row.count / max) * 100, row.count > 0 ? 5 : 0); const content = <><span className="mb-2 text-[0.65rem] font-semibold text-slate-300">{formatCount(row.count)}</span><span className="block w-full rounded-t-md shadow-lg" style={{ height: `${heightPct}%`, minHeight: row.count > 0 ? "0.75rem" : "0", background: `linear-gradient(180deg, ${fill}, rgba(56,189,248,0.35))`, boxShadow: `0 0 18px ${fill}55` }} /><span className="mt-2 h-8 w-full truncate text-center text-[0.62rem] text-slate-400" title={row.label}>{row.label}</span></>; return param ? <Link href={`/cves?${param}=${encodeURIComponent(row.label)}`} key={row.label} className="flex h-full min-w-0 flex-1 flex-col items-center justify-end rounded-md px-1 hover:bg-cyan-400/10">{content}</Link> : <div key={row.label} className="flex h-full min-w-0 flex-1 flex-col items-center justify-end px-1">{content}</div>; })}</div></section>;
}
